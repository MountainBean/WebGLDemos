...
namespace state {
    ...
    sg_pipeline pip_windows;
    sg_bindings bind_cubes;
    sg_bindings bind_plane;
    sg_bindings bind_windows;
...
static void init(void) {
    ...
    std::vector<float> windowsVertices {
        // positions          // texture coords
    ...
    sg_buffer windows_buffer = sg_make_buffer(sg_buffer_desc {
        .size = windowsVertices.size() * sizeof(float),
        .data = sg_range {
            .ptr = windowsVertices.data(),
            .size = windowsVertices.size() * sizeof(float),
        },
        .label = "windows-vertices"
    });

    state::bind_windows.vertex_buffers[0] = windows_buffer;
    ...
    // create a pipeline object (default render states are fine for triangle)
    state::pip_windows = sg_make_pipeline(sg_pipeline_desc {
        .shader = simple_shd,
        .layout = layout,
        .depth {    // Our first 3D elements so we need to enable depth testing
            .compare = SG_COMPAREFUNC_LESS,   // discard fragments that are further away
    ...
    SokTexture windowTex("../data/blending_transparent_window.png",
                          state::bind_windows,
                          IMG__texture1,
                          SMP_texture1_smp,
                          true,
...
void frame(void) {
    ...
    for (size_t i = 0; i < state::windows.size(); i++) {
        float distance = glm::length(state::camera.pos - state::windows[i]);
        sorted[distance] = state::windows[i];
    }

    sg_apply_pipeline(state::pip_windows);
    sg_apply_bindings(state::bind_windows);

    for(std::map<float,glm::vec3>::reverse_iterator it = sorted.rbegin(); it != sorted.rend(); ++it) {
        vs_params.model = glm::translate(glm::mat4(1.0f), it->second);
        sg_apply_uniforms(UB_vs_params, SG_RANGE(vs_params));
        sg_draw(0, 6, 1);
    }
