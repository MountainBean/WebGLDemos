...
void frame() {
    ...
    float now = static_cast<float>(stm_sec(stm_now()));
    float greenValue = (sinf(now) / 2.0f) + 0.5f;

    fs_params_t fs_params {
        .ourColor = {0.0f, greenValue, 0.0f, 1.0f}
    };
    sg_apply_uniforms(UB_fs_params, SG_RANGE_REF(fs_params));
...
sapp_desc sokol_main([[maybe_unused]] int argc, [[maybe_unused]] char* argv[]) {
    ...
    sad.height = 600;
    sad.high_dpi = true;
    sad.window_title = "Uniforms - LearnOpenGL";
    return sad;
}
