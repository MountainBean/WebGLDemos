...
void frame(void) {
    ...
    float camX = sinf(static_cast<float>(stm_sec(stm_now()))) * radius;
    float camY = cosf(static_cast<float>(stm_sec(stm_now()))) * radius;

    glm::mat4 view = glm::lookAt(glm::vec3(camX, 0.0f, camY),
                                 glm::vec3(0.0f, 0.0f, 0.0f),
