...
#include "1-stencil-testing.glsl.h"
...
namespace state {
    ...
    sg_pipeline pip_cube_outlines;
    sg_pipeline pip_plane;
    sg_bindings bind_cubes;
    sg_bindings bind_cube_outlines;
...
static void init(void) {
    ...
    state::bind_cube_outlines.vertex_buffers[0] = cube_buffer;
    ...
    state::pip_cubes = sg_make_pipeline(sg_pipeline_desc {
        ...
        .stencil = {
            ...
            .read_mask = 0xFF, // glStencilFunc mask
            .write_mask = 0xFF, // opengl glStencilMask
            .ref = 1, // glStencilFunc ref 
        },
    ...
    sg_shader outline_shd = sg_make_shader(stencilOutline_shader_desc(sg_query_backend()));

    // create an outline pipeline object
    state::pip_cube_outlines = sg_make_pipeline(sg_pipeline_desc {
        .shader = outline_shd,
        .layout = layout,
        // OpenGL
        // glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);
...
void frame(void) {
    ...
    // Outlines
    sg_apply_pipeline(state::pip_cube_outlines);
    sg_apply_bindings(state::bind_cube_outlines);

    model = glm::translate(glm::mat4(1.0f), glm::vec3(-1.0f, 0.0f, -1.0f));
    vs_params.model = glm::scale(model, glm::vec3(1.2f));
    sg_apply_uniforms(UB_vs_params, SG_RANGE(vs_params));
    sg_draw(0, 36, 1);
    model = glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f));
    vs_params.model = glm::scale(model, glm::vec3(1.2f));
